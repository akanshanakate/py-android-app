...
class MainApp(MDApp):
    ...
    def search(self):
        # if result is not empty make it empty for the next one 
        if self.help_string.get_screen('Wikipedia App').ids.result.text != "":
            self.help_string.get_screen('Wikipedia App').ids.result.text =  ' '

        if self.help_string.get_screen('Wikipedia App').ids.rc_spin.active == True :
